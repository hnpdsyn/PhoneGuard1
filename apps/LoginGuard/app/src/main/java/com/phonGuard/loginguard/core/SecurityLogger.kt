...
/**
 * 安全日志引擎 - 记录所有安全事件
 * v1.1 新增 getAllLogs()：读取全部日志文件并按时间倒序返回，供事件时间线（TimelineScreen）使用
 */
class SecurityLogger(private val context: Context) {
    ...
    /**
     * 读取所有日志（跨全部日志文件），按时间倒序（v1.1 新增）
     * 用于事件时间线（TimelineScreen）：解锁成功/密码输错/自动锁机/拍照取证等
     */
    suspend fun getAllLogs(): List<SecurityEvent> {
        return withContext(Dispatchers.IO) {
            try {
                val files = logDir.listFiles { f ->
                    f.name.startsWith("security_") && f.name.endsWith(".json")
                ...
                // timestamp 格式为 yyyy-MM-dd HH:mm:ss，字符串排序即时间排序
