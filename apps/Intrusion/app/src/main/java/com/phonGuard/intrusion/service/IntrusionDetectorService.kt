package com.phonGuard.intrusion.service
...
class IntrusionDetectorService : Service() {
    ...
    private suspend fun checkIntrusionThreshold() {
        val config = (application as IntrusionApp).configManager
    ...
    private suspend fun handleIntrusionDetected() {
        ...
        NotificationHelper.sendAlertNotification(
            this,
            IntrusionApp.CHANNEL_INTRUSION,
            "🚨 入侵检测告警！",
    ...
    private fun logEvent(type: String, subType: String, message: String) {
        try {
            kotlinx.coroutines.runBlocking {
                val logger = (application as IntrusionApp).securityLogger
                logger.logEvent(
    ...
    companion object {
        ...
        const val ACTION_TEST_INTRUSION = "com.phonGuard.intrusion.action.TEST_INTRUSION"
        const val ACTION_RESET_COUNT = "com.phonGuard.intrusion.action.RESET_INTRUSION_COUNT"
    }
}
